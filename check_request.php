...
<p>-ติดตั้ง firewall และ set ระบบอินเตอร์เน็ต</p> 
            ...
            <div class="row justify-content-start mb-0 mb-xl-3" id="dsk">
                <div class="col-10 col-xl-4 mb-3 mb-xl-0">
                    ...
                    <input type="text" class="data form-control ftitle" name="contact" id="contact" value="คุณสุดใจ"
                        disabled>
                </div>
                <div class="col-10 col-xl-4 mb-3 mb-xl-0">
...
<p>-ติดตั้ง firewall และ set ระบบอินเตอร์เน็ต</p> 
    ...
    <script>
        ...
        const status = <?=$data['Status_form_id']?>; //ค่า status 
        const box = document.getElementById('editBox');
        const topic = box.getElementsByTagName('p');
        var str;
        if (status != 1) {
            if(status == 2){
                str = 'รายละเอียดที่ต้องการแก้ไข โดย ';
            }else if(status == 3){
                str = 'สาเหตุที่ยกเลิก โดย ';
